...
Claude Code startet stdio-MCP-Server **je Session**. basic-memory lädt den fastembed-Cross-Encoder in genau diesen Prozess: gemessen 1,3 GB nach dem Modell-Init und 6,0 GB nach wenigen Suchen, danach konstant (2026-09-12, 0.23.2, `vmmap --summary`). Fünf offene Sessions bedeuteten ~25 GB und einen swappenden Rechner. Seit 0.4.0 läuft deshalb **ein** Server für alle Sessions.

- `install.py` schreibt zwei LaunchAgents und kopiert `watchdog.py` nach `~/.local/share/mastermind/bin/`. Die Laufzeitdateien liegen bewusst **außerhalb** des Plugin-Caches: dessen Pfad enthält die Version und würde bei jedem Update den launchd-Job brechen.
  - `com.mastermind.basic-memory`: `basic-memory mcp --transport streamable-http --host 127.0.0.1 --port 8765`, `RunAtLoad` plus `KeepAlive` (startet nach Login und nach Absturz), Log `~/.local/state/mastermind/server.log`.
  - `com.mastermind.basic-memory-watchdog`: alle 600 s `watchdog.py`.
- `python3 plugins/mastermind/server/install.py [--status|--uninstall]`. `install.py` ist idempotent, macht einen echten MCP-`initialize` als Health-Check und meldet stdio-Server, die noch aus alten Sessions laufen.
- Der Watchdog startet den Dienst **nur** neu, wenn der Footprint über 1500 MB liegt **und** kein `claude`-Prozess läuft. Der zweite Teil ist die eigentliche Sicherung: streamable-http vergibt eine Session-ID, die ein neu gestarteter Server nicht kennt — ein Neustart bei offenen Sessions würde deren Memory-Tools still brechen. Offene Sessions halten den Speicher also bewusst; mit dem letzten geschlossenen Terminal fällt der Dienst auf ~200 MB zurück.
- Port 8765 steht an zwei Stellen und muss zusammenpassen: `PORT` in `install.py` und die `url` in `plugins/mastermind/.mcp.json`.
