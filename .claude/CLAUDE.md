...
- `.mcp.json` verweist per `type: http` auf **einen geteilten Server** unter `http://127.0.0.1:8765/mcp` statt einen stdio-Server je Session zu starten (seit 0.4.0, siehe „Der geteilte Server" unten). Die Tools erscheinen in Claude Code weiterhin als `mcp__plugin_mastermind_mastermind-memory__<tool>`. Der Vault liegt bewusst **nicht** unter `~/Desktop` (TCC kann Hintergrundprozessen den Zugriff verweigern, basic-memory hält dann alle Notizen für gelöscht).
...
- `skills/mastermind-index/` (`name: index` → `/mastermind:index [fix] [repair-index]`, ersetzt das frühere `commands/index.md`): `lint.py` prüft Frontmatter, Typ/Ordner, Titel, Quoting, Wertemengen, Datumsregeln, `source`, Stack-Tags, Wikilink-Ziele, Orphans, Hub-Anatomie v3, Root-Zeilenlimits (`user.md` ≤ 60, `soul.md` ≤ 40), Inbox und den Index (`~/.basic-memory/memory.db` read-only). Bestandsschutz: Pflichtfelder, Wertemengen, `source` und Datumsregeln sind ERROR nur ab `created ≥ 2026-09-04`, sonst WARN; Titelregeln immer WARN; `templates/`, `_brainstorming/`, `_*` ausgenommen, `inbox/` nur INFO. CLI `[--vault] [--changed] [--json] [--strict] [--index-only] [--fix] [PFAD …]`; `--fix` nur sichere Fixes (`related`, Quoting, `## Verlauf`, `## Quellen`). `repair_index.py` repariert den Volltext-Index über den laufenden Watcher — seit 0.4.0 in der Regel der geteilte Dienst, sonst ein stdio-Server (Checksum `NULL` + `touch` in 25er-Batches, max. 5 Runden, Sicherung nach `~/.basic-memory/backups/repair/`, höchstens 5 Sicherungen dort); ohne Watcher Meldung mit `launchctl kickstart`-Hinweis, bei > 2 Watchern Abbruch (`--force`).
...
- **Index-Reparatur-Regel:** `basic-memory reindex` (0.22.1 und 0.23.2, alle Varianten) schreibt Titel-only-Zeilen in den FTS-Index und verliert Observation-/Relation-Zeilen; `reindex --full --embeddings` bläht die Vektortabelle auf. Nur der Watcher des laufenden MCP-Servers indexiert vollständig. Deshalb: nie `reindex --full` auf einen gesunden Index, Reparatur über `/mastermind:index repair-index`; ein Modellwechsel läuft über den Server-Pfad (`retrieval-upgrade.sh`, untracked in `.claude/nachtsessions/`, startet kurz einen eigenen `basic-memory mcp`). Konkurrierende Watcher erzeugen doppelte FTS-Zeilen und `database is locked`; seit 0.4.0 läuft durch den geteilten Server normalerweise **genau ein** Watcher, `repair_index.py` meldet im Kopf, ob es der Dienst oder ein stdio-Rest ist. Sessions, die noch auf stdio stehen (alter Plugin-Stand), vor Reparatur und Nachtsessions schließen.
- Lokale Voraussetzungen: `uv`, `basic-memory` als uv-Tool (0.23.2, Installation mit `--prerelease=allow`), registriertes Projekt (`basic-memory project list` muss `mastermind` mit Pfad `~/Mastermind` zeigen), `python3`, macOS mit `launchctl`. Semantische Suche läuft über Ollama (`brew services start ollama`, LaunchAgent `homebrew.mxcl.ollama.plist`) mit `qwen3-embedding:8b` (4096 Dimensionen, LiteLLM-Provider, Query-Präfix, Schwelle 0.60) plus fastembed-Reranker `jinaai/jina-reranker-v2-base-multilingual` über 20 Kandidaten (Cache `~/.basic-memory/fastembed_cache`, 2,2 GB). `auto_update` bleibt `false`. Der SQLite-Index liegt unter `~/.basic-memory/`, nicht im Vault.

### Der geteilte Server (`plugins/mastermind/server/`)

Claude Code startet stdio-MCP-Server **je Session**, und basic-memory lädt den fastembed-Cross-Encoder in genau diesen Prozess. Seit 0.4.0 läuft deshalb **ein** Server für alle Sessions; seit 0.5.0 hält er seinen Speicher selbst flach und übersteht Neustarts ohne Session-Verlust (Design: `docs/superpowers/specs/2026-09-12-mastermind-shared-server-design.md` und `…/2026-09-16-mastermind-reranker-memory-design.md`).

- `install.py` kopiert `server.py` und `watchdog.py` nach `~/.local/share/mastermind/bin/` und schreibt zwei LaunchAgents. Die Laufzeitdateien liegen bewusst **außerhalb** des Plugin-Caches: dessen Pfad enthält die Version und würde bei jedem Update den launchd-Job brechen.
  - `com.mastermind.basic-memory`: `<venv-python> server.py mcp --transport streamable-http --host 127.0.0.1 --port 8765` mit `FASTMCP_STATELESS_HTTP=true`, `RunAtLoad` plus `KeepAlive` (startet nach Login und nach Absturz), Log `~/.local/state/mastermind/server.log`.
  - `com.mastermind.basic-memory-watchdog`: alle 600 s `watchdog.py`.
- `server.py` startet die normale basic-memory-CLI, patcht vorher aber den Reranker-Pfad; Suchergebnisse bleiben identisch, nur die Allokation ändert sich: ONNX-Arena aus (`enable_cpu_mem_arena=False`, die einzige Session-Option, die fastembed durchreicht), ein Lock serialisiert die Reranks, `batch_size` 4 statt 64, und nach 600 s ohne Suche wird das Modell entladen (Neuladen 0,6 s). Grund, gemessen 2026-09-15: Die Arena gibt Aktivierungsspeicher nie zurück und wächst bei wechselnden Eingabeformen — 1,3 GB nach dem Modell-Load, 3,9 GB nach zwei Suchen, 5,6 GB nach zwei parallelen; drei parallele `/mastermind:wrap` trieben den Dienst auf 17,5 GB. Jetzt: ~200 MB ohne Modell, ~1,7 GB mit geladenem Modell, Spitze bei drei parallelen Suchen 2,1 GB, danach zurück. Jeder Patch prüft, was er ersetzt; passt basic-memory oder fastembed nicht mehr, startet der Server ungepatcht und meldet es in `server.log` (`[mastermind-server] start: patched=False …`). Stellknöpfe per Env: `MASTERMIND_RERANK_BATCH`, `MASTERMIND_RERANK_IDLE_UNLOAD_S`, `MASTERMIND_RERANK_ARENA`.
- Stateless-HTTP: Der Server vergibt keine `Mcp-Session-Id` mehr und ignoriert mitgeschickte; ein Neustart ist für offene Claude-Sessions unsichtbar, der nächste Aufruf landet auf dem neuen Prozess. Deshalb darf der Watchdog neu starten, sobald der Footprint über 3000 MB liegt **und** der Dienst leer läuft (keine offene Verbindung auf dem Port, letzte Anfrage laut `server.log` vor ≥ 300 s). Die frühere Bedingung „kein `claude`-Prozess" trat auf einem Rechner mit tagelang offenen Terminals nie ein (Watchdog-Log 2026-09-15: 17,5 GB, „8 Claude session(s) open; keeping it warm" über Stunden).
- `python3 plugins/mastermind/server/install.py [--status|--uninstall]`. `install.py` ist idempotent, macht einen echten MCP-`initialize` als Health-Check, meldet den Session-Modus (`stateless`) und die letzten `[mastermind-server]`-Zeilen und listet stdio-Server aus alten Sessions.
- Port 8765 steht an zwei Stellen und muss zusammenpassen: `PORT` in `install.py` (der Watchdog bekommt ihn per `MASTERMIND_PORT`) und die `url` in `plugins/mastermind/.mcp.json`.
- Der SessionStart-Hook prüft den Endpunkt per TCP und warnt mit dem `launchctl kickstart`-Befehl, wenn nichts antwortet. Er liest dafür die `.mcp.json` des Plugins; auf stdio zurückgestellt verschwindet die Warnung von selbst.
- Ollama bleibt der zweite große Posten: `qwen3-embedding:8b` belegt geladen rund 6 GB, wird nach 5 min Leerlauf entladen und brauchte unter Speicherdruck 10–44 s je Notiz (basic-memory.log 2026-09-15). Das ist durch die Retrieval-Decision vom 2026-09-03 gesetzt und hier nicht angefasst.
