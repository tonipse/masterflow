...
## RECALL first (pull)
At the start of a non-trivial coding task, BEFORE implementing, search the brain with the task's key terms (tech, error, domain) using the `search` tool (semantic/hybrid). If relevant notes exist, read them (`read_note` / `build_context`), factor them in, and tell the user which prior knowledge you are reusing.
...
2. `search` first to avoid duplicates — prefer `edit_note` on an existing note over a near-duplicate.
...
`write_note, read_note, edit_note, move_note, search, recent_activity, build_context, list_directory`.
